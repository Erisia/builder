...
def main():
    ...
    if args.work_dir:
        ...
        work = Path(tempfile.mkdtemp(prefix="erisia-save-threading-test-"))
    (work / "mods").mkdir()
    test_jar = args.build / f"erisia-save-threading-tests-{VERSION}.jar"
    shutil.copy2(test_jar, work / "mods" / test_jar.name)
    if not args.without_fix:
        fix_jar = args.build / f"erisia-save-threading-{VERSION}.jar"
    ...
    command = [java, "-Xms256m", "-Xmx1g", "-Djava.net.preferIPv4Stack=true",
               ...
               "-Derisia.test.case=" + args.case, "-jar", str(launcher / "cleanroom-0.6.12-alpha.jar"), "nogui"]
